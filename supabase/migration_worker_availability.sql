-- Allow active workers to pause receiving new jobs without changing approval status.

ALTER TABLE public.workers
ADD COLUMN IF NOT EXISTS is_available BOOLEAN NOT NULL DEFAULT TRUE;

CREATE INDEX IF NOT EXISTS workers_active_available_idx
  ON public.workers(status, is_available)
  WHERE status = 'active';
...
BEGIN
  ...
  IF v_worker.is_available IS FALSE THEN
    RAISE EXCEPTION 'WORKER_OFFLINE' USING ERRCODE = 'P0001';
  END IF;
