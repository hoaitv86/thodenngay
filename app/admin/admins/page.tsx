...
export default function AdminAccountsPage() {
  ...
  const selectedAccount = useMemo(
    () => accounts.find((account) => account.id === form.userId) || null,
    [accounts, form.userId],
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {message && (
        <div
          className={`fixed right-4 top-4 z-50 flex max-w-sm items-start gap-3 rounded-lg border px-4 py-3 text-sm font-bold shadow-elevated ${
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-body-sm text-on-surface-variant">
            Quản lý tài khoản đăng nhập admin, tên hiển thị, email và mật khẩu.
          </p>
        </div>
            ...
            <div className="divide-y divide-outline-variant/20">
              {accounts.map((account) => (
                <button
                  ...
                  className={`block w-full p-4 text-left transition-colors hover:bg-surface-container-low ${
                    selectedAccount?.id === account.id ? "bg-primary-fixed/50" : ""
                  }`}
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {account.is_super_admin && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-primary-fixed px-2 py-1 text-[10px] font-extrabold uppercase text-primary">
                    ...
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-extrabold uppercase ${
                        account.status === "blocked" ? "bg-error-container text-error" : "bg-success-container text-success"
                      }`}
                    >
        ...
        <form onSubmit={saveAccount} className="rounded-lg border border-outline-variant/30 bg-white p-4">
          <div className="flex items-center gap-3 border-b border-outline-variant/30 pb-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary-fixed text-primary">
              {editing ? <UserCog className="h-5 w-5" /> : <ShieldCheck className="h-5 w-5" />}
            </div>
            <div>
              <h2 className="font-extrabold text-on-surface">{editing ? "Cập nhật admin" : "Tạo admin mới"}</h2>
              <p className="text-xs text-on-surface-variant">
                {editing ? "Đổi email đăng nhập, tên hiển thị hoặc mật khẩu." : "Tài khoản mới sẽ có vai trò admin."}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-4">
            ...
            <label className="flex items-start gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low p-3 text-sm">
              <input
                ...
                onChange={(event) => setForm((prev) => ({ ...prev, isSuperAdmin: event.target.checked }))}
              ...
              <span>
                ...
                <span className="mt-1 block text-xs leading-5 text-on-surface-variant">
                  Super Admin có toàn quyền quản lý admin khác và các thiết lập bảo mật.
                </span>
              </span>
            </label>
            {currentIsSuperAdmin && (
          ...
          <div className="mt-5 flex justify-end gap-3 border-t border-outline-variant/30 pt-4">
            {editing && (
              <button type="button" onClick={startCreate} className="btn-outline !w-auto !px-4 !py-2" disabled={saving}>
                Hủy
              </button>
            )}
            <button className="btn-primary !w-auto !px-5 !py-2" disabled={saving}>
              {saving ? (
                "Đang lưu..."
              ) : (
                <>
                  <LockKeyhole className="h-4 w-4" />
                  {editing ? "Lưu thay đổi" : "Tạo admin"}
                </>
              )}
            </button>
          </div>
