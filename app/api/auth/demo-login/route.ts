...
const getFallbackDemoEmail = (role: DemoRole) =>
  role === "customer"
...
const signInWithFallbackDemoAccount = async (role: DemoRole) => {
  ...
  if (authError || !authData.session || !authData.user) {
    return NextResponse.json(
      {
        error:
          "Không thể đăng nhập demo. Hãy tạo tài khoản " +
          `${demoEmail} với mật khẩu demo hoặc seed tài khoản demo ${demoAccount.phone}.`,
      },
      { status: 500 }
    );
  }
