...
useEffect(() => {
  ...
  const handleConfirmCompleteJob = async () => {
    ...
    try {
      ...
      setDashboardData(prev => ({
        ...prev,
        newJobs: prev.newJobs.filter(j => j.id !== job.id),
        pendingApprovalJobs: prev.pendingApprovalJobs.filter(j => j.id !== job.id),
                  ...
                  return (
                    ...
                    >
                      <Link
                        ...
                        className="block rounded-lg transition-colors active:bg-surface-container-low"
                      >
